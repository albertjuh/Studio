...
const formSchema = z.object({
  ...
  numberOfCartons: z.coerce.number().positive("Number of cartons must be a positive number."),
      ...
      >
        ...
        <FormStep>
            <FormField control={form.control} name="receiptDate" render={({ field }) => (<FormItem className="flex flex-col"><FormLabel>When were they received?</FormLabel>
                ...
                <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value} onSelect={field.onChange} disabled={(date) => date > new Date() && date.toDateString() !== new Date().toDateString()} initialFocus /></PopoverContent>
                </Popover><FormMessage />
            </FormItem>)} />
        </FormStep>
         <FormStep isOptional>
