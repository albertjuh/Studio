...
import { saveNyangaReportAction, addNyangaWorkerAction } from '@/lib/nyanga-actions';
import { Button } from '@/components/ui/button';
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from '@/components/ui/form';
import { Input } from '@/components/ui/input';
import { useToast } from '@/hooks/use-toast';
import { Save, CalendarIcon, PlusCircle, X, Loader2, UserPlus, AlertCircle } from 'lucide-react';
...
interface DailyReportFormProps {
  ...
  isErrorWorkers: boolean;
}

const ADD_NEW_WORKER_VALUE = "__add_new_worker__";

export function DailyReportForm({ supervisorId, workers, isLoadingWorkers, isErrorWorkers }: DailyReportFormProps) {
  ...
  const addWorkerMutation = useMutation({
    ...
    onSuccess: (result) => {
      if(result.success && result.newWorker) {
        toast({ title: 'Worker Added', description: `Successfully added ${result.newWorker.name}.` });
        queryClient.invalidateQueries({ queryKey: ['nyangaWorkers'] });
        
        const kgValue = parseFloat(newItem.kg);
        if (kgValue > 0) {
            append({ workerName: result.newWorker.name, kg: kgValue });
        }
  ...
  return (
    <Card>
      <CardHeader>
          ...
          <CardDescription>Select the date and shift, then use the floating button to add entries for each worker's production.</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
