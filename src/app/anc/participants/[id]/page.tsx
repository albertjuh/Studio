    ...
    <div className="max-w-4xl mx-auto space-y-4 pb-12">
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="space-y-0.5">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Badge className="bg-primary/10 text-primary border-none font-black text-[7px] px-1.5 h-4 uppercase">
      ...
      <div className="grid gap-4 lg:grid-cols-12">
        <div className="lg:col-span-8 space-y-4">
          <Card className="border-none ring-1 ring-border shadow-sm rounded-2xl overflow-hidden bg-card/60 backdrop-blur-sm">
            ...
            <CardContent className="p-3 grid grid-cols-2 md:grid-cols-4 gap-2">
                {surveyItems.map((s) => (
                    <div key={s.num} className={cn(
                        "p-2 rounded-xl border-2 transition-all h-full flex flex-col justify-between",
                        s.done ? "border-primary/20 bg-primary/5" : (s.attempted && !s.done ? "border-amber-200 bg-amber-50/50" : "border-border/50 bg-muted/20")
                    )}>
                        ...
                        <div className="mt-2 space-y-0.5">
                            ...
                            <p className="text-[8px] font-black text-primary/70 leading-tight">
                                {s.done ? safeFormatDate(s.actual) : (s.attempted ? 'INC' : 'Pending')}
                            </p>
                        </div>
                    </div>
                ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="p-2 rounded-xl bg-primary/5 border border-dashed border-primary/20 flex flex-col gap-1">
                            {activeP.nextOfKinName && <p className="text-[10px] font-black text-primary leading-tight">{activeP.nextOfKinName}</p>}
                            <p className="font-mono font-black text-xs text-primary/70 leading-none">{activeP.alternativeContact || 'N/A'}</p>
                        </div>
                    </div>
                </div>
            </CardContent>
        ...
        <div className="lg:col-span-4 space-y-4">
            <Card className="border-none ring-1 ring-border shadow-sm rounded-2xl overflow-hidden bg-card/60 backdrop-blur-sm h-full">
                    ...
                    <ScrollArea className="h-[500px]">
                        {rawEvents && rawEvents.length > 0 ? (
                            <div className="divide-y divide-border/40">
                                {rawEvents.map((event) => (
                                    <div key={event.id} className="p-4 space-y-2 hover:bg-muted/10 transition-all">
                                        <div className="flex items-center gap-2">
                                            <div className="p-1.5 rounded-lg bg-primary/10 text-primary">
                                                {event.event_type === 'enrolled' ? <UserPlus className="h-3.5 w-3.5" /> : <Phone className="h-3.5 w-3.5" />}
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-black uppercase tracking-tight leading-none">{event.event_type.replace('_', ' ')}</p>
                                                <p className="text-[8px] font-bold text-slate-400 mt-0.5">{safeFormatDate(event.event_date)}</p>
                                            </div>
                                        </div>
                                        {event.notes && <p className="text-[10px] font-medium text-slate-600 italic bg-slate-50 p-2 rounded-lg border border-dashed">"{event.notes}"</p>}
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div className="p-12 text-center text-slate-400 italic font-bold text-[10px] uppercase tracking-widest">No audit events found</div>
                        )}
                    </ScrollArea>
                </CardContent>
            </Card>
      ...
      <div className="p-4 bg-muted/30 border-2 border-dashed rounded-2xl text-center">
          <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">
              Audit-only Dossier. Updates are managed via the <Link href="/anc/survey2-calls" className="text-primary hover:underline font-black">Survey 2 Call Plan</Link>.
          </p>
      </div>
    </div>
    );
